    ...
    try:
        for fmt in load_formats(args.formats):
            for taxonomy_term, search_query in expand(fmt):
                token, complete = store.query_state(fmt.slug, search_query)
                if complete: continue
                for _ in range(args.pages_per_query):
                    page = api.search(search_query, token); added += store.add_search_videos(page.get("items", []), fmt.slug, taxonomy_term)
                    token = page.get("nextPageToken")
                    if not token: store.save_query_state(fmt.slug, search_query, None, True); break
                    store.save_query_state(fmt.slug, search_query, token, False)
    finally: store.close()
...
p = sub.add_parser("collect"); p.add_argument("--database", default="corpus.sqlite3"); p.add_argument("--formats", default="config/formats.yaml"); p.add_argument("--pages-per-query", type=int, default=1); p.set_defaults(func=collect)
